...
GITHUB_REPO_MARZBAN="bsntire/Marzbantest"
GITHUB_REPO_SCRIPTS="bsntire/vpn-bot"
GITHUB_BRANCH="main"  # или "master", в зависимости от вашей ветки
FILES_URL_PREFIX="https://raw.githubusercontent.com/$GITHUB_REPO_MARZBAN/$GITHUB_BRANCH"
SCRIPTS_URL_PREFIX="https://raw.githubusercontent.com/$GITHUB_REPO_SCRIPTS/$GITHUB_BRANCH"
...
install_marzban_script() {
    # Используем ваш репозиторий scripts для установки marzban.sh
    SCRIPT_URL="$SCRIPTS_URL_PREFIX/marzban.sh"
    colorized_echo blue "Installing marzban script from $GITHUB_REPO_SCRIPTS"
...
install_marzban() {
    ...
    # Используем ваш GitHub репозиторий вместо оригинального
...
EOF
        ...
        # Modify .env file - используем ваш GitHub
        colorized_echo blue "Fetching .env file from $GITHUB_REPO_MARZBAN"
...
EOF
        ...
        echo "----------------------------"
        colorized_echo blue "Fetching compose file from $GITHUB_REPO_MARZBAN"
    ...
    colorized_echo blue "Fetching xray config file from $GITHUB_REPO_MARZBAN"
    ...
    colorized_echo green "Marzban's files downloaded successfully from $GITHUB_REPO_MARZBAN"
}
...
install_command() {
    ...
    detect_compose
    install_marzban_script
    # Function to check if a version exists in the GitHub releases - используем ваш репозиторий
    check_version_exists() {
        local version=$1
        # Используем ваш репозиторий Marzban API
        repo_url="https://api.github.com/repos/$GITHUB_REPO_MARZBAN/releases"
        ...
        if echo "$response" | jq -e ".[] | select(.tag_name == \"${version}\")" > /dev/null 2>&1; then
            return 0
        else
            return 1
        fi
    ...
    if [[ "$marzban_version" == "latest" || "$marzban_version" == "dev" || "$marzban_version" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        if check_version_exists "$marzban_version"; then
            ...
            echo "Version $marzban_version does not exist in $GITHUB_REPO_MARZBAN. Please enter a valid version (e.g. v0.5.2)"
            exit 1
        fi
    else
...
update_marzban_script() {
    # Используем ваш репозиторий scripts для обновления скрипта
    SCRIPT_URL="$SCRIPTS_URL_PREFIX/marzban.sh"
    colorized_echo blue "Updating marzban script from $GITHUB_REPO_SCRIPTS"
    curl -sSL $SCRIPT_URL | install -m 755 /dev/stdin /usr/local/bin/marzban
...
usage() {
    ...
    colorized_echo cyan "Marzban Repository: $GITHUB_REPO_MARZBAN"
    colorized_echo cyan "Scripts Repository: $GITHUB_REPO_SCRIPTS"
    colorized_echo blue "================================"
    echo
}
